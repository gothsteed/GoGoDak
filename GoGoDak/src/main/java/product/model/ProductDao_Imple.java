...
public class ProductDao_Imple implements ProductDao {
	...
	private Product_detailVO createProduct_detailVO(ResultSet rs) throws SQLException {
		Product_detailVO product_detailVO = new Product_detailVO();
		
		product_detailVO.setProduct_detail_seq(rs.getInt("PRODUCT_DETAIL_SEQ"));
		product_detailVO.setFk_product_seq(rs.getInt("FK_PRODUCT_SEQ"));
		product_detailVO.setDetail_name(rs.getString("DETAIL_NAME"));
		product_detailVO.setStock(rs.getInt("STOCK"));
	...
	public Pager<ProductVO> getProductByType(int type, int currentPage, int blockSize) throws SQLException {
		...
		try {
			...
			String sql = "    SELECT * " + "    FROM " + "    ( "
					...
					+ "    DISCOUNT_TYPE,  DISCOUNT_NUMBER " + "        FROM " + "        ( " + "            select * "
					+ "            from tbl_product " + "            where product_type = ? and exist_status = 1) V ";
			...
			String totalScountSql = " select count(*) " + "            from tbl_product "
	...
	public ProductVO getProductBySeq(int product_seq) throws SQLException {
		...
		try {
			...
			String sql = " select * " + " from tbl_product " + " where product_seq = ? ";
	...
	public Pager<ProductVO> getProductList(String searchWord, int currentPage, int blockSize) throws SQLException {
		...
		try {
			...
			String sql = "    SELECT * " + "    FROM " + "    ( "
					+ "        SELECT rownum as rno, PRODUCT_SEQ, FK_MANUFACTURER_SEQ,"
					+ "   PRODUCT_NAME, DESCRIPTION, BASE_PRICE,  STOCK, MAIN_PIC, DISCRIPTION_PIC, PRODUCT_TYPE, FK_DISCOUNT_EVENT_SEQ,  "
					+ "    DISCOUNT_TYPE,  DISCOUNT_NUMBER " + "        FROM " + "        ( "
					+ "            select rownum as rno,  " + "       product_seq,  " + "       fk_manufacturer_seq,  "
					+ "       product_name,  " + "       description,  " + "       base_price,  " + "       stock,  "
					+ "       main_pic,  " + "       discription_pic,  " + "       product_type,  "
					+ "       discount_type,  " + "       discount_number, " + "       fk_discount_event_seq "
					+ "from tbl_product " + "where product_name like '%' || ? || '%'  " + "  and exist_status = 1 "
					+ ") V ";
	...
	// 상품등록
	@Override
	public int productregister(ProductVO pvo, String[] product_detail, Integer[] detail_stock) throws Exception {
		...
		try {
			...
			pstmt = conn.prepareStatement(sql,  new String[] {"product_seq"});
			...
			String sqlProductDetail = "INSERT INTO tbl_product_detail (PRODUCT_DETAIL_SEQ, FK_PRODUCT_SEQ, DETAIL_NAME, stock) VALUES (PRODUCT_DETAIL_SEQ.nextval,?,  ?, ?)";
			pstmt = conn.prepareStatement(sqlProductDetail);

			for (int i = 0; i < product_detail.length; i++) {
				if(product_detail[i].trim().isEmpty() || detail_stock[i] == null) {
					continue;
				}
				
				pstmt.setInt(1, product_seq);
				pstmt.setString(2, product_detail[i]);
				pstmt.setInt(3, detail_stock[i]);
				pstmt.addBatch();
			}
	...
	@Override
	public Pager<ProductVO> getProductByDiscountEvent(int discount_event_Seq, int currentPage, int blockSize)
			throws SQLException {
		List<ProductVO> productList = new ArrayList<>();
		int totalPage = 0;
		...
		try {
			...
			String totalPageSql = " select count(*) " + "            from tbl_product "
	...
	@Override
	public Pager<ProductVO> getBrandProductList(String manufacturer_seq, int currentPage, int blockSize)
			throws SQLException {
		...
		try {
			...
			String sql = " rownum as rno, manufacturer_name, product_seq, fk_manufacturer_seq, product_name, description, base_price, stock, main_pic, discription_pic, product_type, discount_type, discount_number, fk_discount_event_seq "
					...
					+ " FROM " + " ( " + "    select manufacturer_seq, manufacturer_name "
					+ "    from tbl_manufacturer " + "    where manufacturer_seq = ? " + " ) M "
					+ " JOIN tbl_product P " + " ON P.fk_manufacturer_seq = M.manufacturer_seq"
					+ " where exist_status = 1 " + " order by product_seq DESC) A" + " WHERE A.rno between ? and ?   ";
			...
			String totalScountSql = " SELECT count(*)  " + "	 FROM   " + "	(   "
					+ "	   select manufacturer_seq, manufacturer_name   " + "	   from tbl_manufacturer   "
					+ "	  where manufacturer_seq = ?   " + "	 ) M   " + "	 JOIN tbl_product P   "
					+ "	 ON P.fk_manufacturer_seq = M.manufacturer_seq  " + "	 where exist_status = 1   "
	...
	// 상품수정하기
	@Override
	public int updateProduct(ProductVO pvo, String[] product_detail, Integer[] detail_stock) throws SQLException {
		int result = 0;
		...
		try {
			...
			System.out.println("FK_PRODUCT_SEQ : " + pvo.getProduct_seq());
			
			sql = " DELETE FROM tbl_product_detail "
					+ " WHERE FK_PRODUCT_SEQ = ? ";
			
			pstmt = conn.prepareStatement(sql);
			pstmt.setInt(1, pvo.getProduct_seq());
			result = pstmt.executeUpdate();
			
			
			sql = "INSERT INTO tbl_product_detail (PRODUCT_DETAIL_SEQ, FK_PRODUCT_SEQ, DETAIL_NAME, stock) VALUES (PRODUCT_DETAIL_SEQ.nextval,?,  ?, ?)";
			pstmt = conn.prepareStatement(sql);
...
//select 
	...
	public int productSelectBySeq(int product_seq) throws Exception {
		...
		try {
			...
			String sql = " select product_seq " + " from tbl_product " + " where product_seq = ? and exist_status = 1 ";
	...
	public int deletedProduct(ProductVO productDelete) throws Exception {
		...
		try {
			conn = ds.getConnection();
			String sql = " UPDATE tbl_PRODUCT " + "SET exist_status = 0 " + "WHERE product_seq = ?";
			pstmt = conn.prepareStatement(sql);
	...
	public List<Map<String, String>> selectStoreMap() throws SQLException {
		...
		try {
			...
			String sql = " select manufacturer_seq,manufacturer_name,manufacturer_tel,location,manufacturer_url,manufacturer_img,lat,lng "
					+ " from tbl_manufacturer " + " order by manufacturer_seq asc ";
	...
	}// end of public List<Map<String, String>> selectStoreMap() throws
		// SQLException-----------------
	...
	}// public List<Map<String, String>> selectStoreMapByLocation(String
		// locationParam) throws SQLException-------------
	...
	public List<ProductVO> getDiscountProduct() throws SQLException {
		...
		try {
			...
			String sql = " select * " + " from tbl_product" + " where discount_type is not null and exist_status = 1 ";
	...
	public List<ProductVO> getRecentProduct() throws SQLException {
		...
		try {
			...
			String sql = "SELECT * " + "FROM ( " + "    SELECT * " + "    FROM tbl_product "
					+ "    ORDER BY PRODUCT_SEQ DESC " + ") " + "WHERE ROWNUM <= 3 and exist_status = 1 ";
	...
	public int insertProductLike(int member_seq, int product_seq) throws SQLException {
		...
		try {
			conn = ds.getConnection();
			String sql = " INSERT INTO tbl_product_like (FK_MEMBER_SEQ, FK_PRODUCT_SEQ) " + "VALUES (?, ?) ";
	...
	public int deletedProductLike(int member_seq, int product_seq) throws SQLException {
		...
		try {
			conn = ds.getConnection();
			String sql = " DELETE FROM tbl_product_like " + "WHERE FK_MEMBER_SEQ = ? AND FK_PRODUCT_SEQ = ? ";
	...
	public int getLikeCount(int product_seq) throws SQLException {
		...
		try {
			...
			String sql = "SELECT count(*) " + " from tbl_product_like" + " where fk_product_seq = ?  ";
	...
	public boolean isLiked(int member_seq, int product_seq) throws SQLException {
		...
		try {
			...
			String sql = "SELECT count(*) " + " from tbl_product_like"
	...
	public List<ProductVO> getLikedProduct(int member_seq) throws SQLException {
		...
		try {
			...
			String sql = " select * " + " from tbl_product a " + " join " + " tbl_product_like b "
					+ " on a.product_seq = b.fk_product_seq " + " where b.fk_member_seq = ? and exist_status = 1 ";
	...
	@Override
	public List<Product_detailVO> getProductDetails(int product_seq) throws SQLException {
		
		List<Product_detailVO> product_detailList = new ArrayList<>();
		...
		try {
			...
			while (rs.next()) {
				...
				Product_detailVO product_detail = createProduct_detailVO(rs);

				product_detailList.add(product_detail);
			} // end of while(rs.next())---------------------
		...
		return product_detailList;
	}

	@Override
	public ProductVO getProductBySeq(int product_seq, int product_detail_seq) throws SQLException {	
		ProductVO productVO = null;
		...
		try {
			...
			sql = " SELECT "
					...
					+ "    tbl_product_detail "
					+ " WHERE "
					+ "    FK_PRODUCT_SEQ = ? and  product_detail_seq = ? " ;
			
			pstmt = conn.prepareStatement(sql);
			pstmt.setInt(1, product_seq);
			pstmt.setInt(2, product_detail_seq);
			
			rs = pstmt.executeQuery();
			if(rs.next()) {
				productVO.setProduct_detailVO(createProduct_detailVO(rs));
			}
	...
	public List<ProductVO> getProductList(int order_seq) throws SQLException {
		...
		try {
			...
			while(rs.next()) {
				...
				product.setProduct_seq(rs.getInt("PRODUCT_SEQ"));
				product.setFk_manufacturer_seq(rs.getInt("FK_MANUFACTURER_SEQ"));
				product.setProduct_name(rs.getString("PRODUCT_NAME"));
				product.setDescription(rs.getString("DESCRIPTION"));
				product.setBase_price(rs.getFloat("BASE_PRICE"));
				product.setStock(rs.getInt("p_stock"));
				product.setMain_pic(rs.getString("MAIN_PIC"));
				product.setDescription_pic(rs.getString("DISCRIPTION_PIC"));
				product.setProduct_type(rs.getInt("PRODUCT_TYPE"));
				product.setDiscount_type(rs.getString("DISCOUNT_TYPE"));
				product.setDiscount_amount(rs.getFloat("DISCOUNT_NUMBER"));
				product.setQuantity(rs.getInt("QUANTITY"));
				
				
				int product_detail_seq = rs.getInt("product_detail_seq");
				if(!rs.wasNull()) {
					Product_detailVO product_detailVO = new Product_detailVO();
					...
					product_detailVO.setFk_product_seq(rs.getInt("FK_PRODUCT_SEQ"));
					product_detailVO.setDetail_name(rs.getString("DETAIL_NAME"));
					product_detailVO.setStock(rs.getInt("PD_STOCK"));
					
					product.setProduct_detailVO(product_detailVO);
				}
